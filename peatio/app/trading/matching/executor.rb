  ...
  private
    ...
    def create_trade_and_strike_orders
      ...
      ActiveRecord::Base.transaction do
        ...
        ask_fee = strike(@trade, @ask, accounts_table["#{@ask.ask}:#{@ask.member_id}"], accounts_table["#{@ask.bid}:#{@ask.member_id}"])
        bid_fee = strike(@trade, @bid, accounts_table["#{@bid.bid}:#{@bid.member_id}"], accounts_table["#{@bid.ask}:#{@bid.member_id}"])
        ...
        if @ask.member.ref_member_id
          Reward.create(member_id: @ask.member.ref_member_id, currency_id: @ask.bid, amount: ask_fee * 0.25, usd_amount: 0, ref_type: 'ReferralReward', trade_id: @trade.id, from_member_id: @ask.member_id)
          Account.find_by(member_id: @ask.member.ref_member_id, currency_id: @ask.bid).plus_funds!(ask_fee * 0.25)
        end
        if @bid.member.ref_member_id
          Reward.create(member_id: @bid.member.ref_member_id, currency_id: @bid.ask, amount: bid_fee * 0.25, usd_amount: 0, ref_type: 'ReferralReward', trade_id: @trade.id, from_member_id: @bid.member_id)
          Account.find_by(member_id: @bid.member.ref_member_id, currency_id: @bid.ask).plus_funds!(bid_fee * 0.25)
        end
      end
    end
    ...
    def strike(trade, order, outcome_account, income_account)
      outcome_value, income_value, currency = OrderAsk === order ? [trade.volume, trade.funds, order.bid] : [trade.funds, trade.volume, order.ask]
